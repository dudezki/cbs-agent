...
<template>
  <div
    class="grid grid-cols-1 md:grid-cols-2 h-screen w-full bg-white dark:bg-gray-900 border-x border-gray-200 dark:border-gray-700 transition-colors duration-300">
    <!-- Left Column: Branding & Marketing -->
    <div
      class="hidden md:flex flex-col justify-between p-12 bg-gray-50 dark:bg-gradient-to-br dark:from-gray-900 dark:via-gray-800 dark:to-black text-gray-900 dark:text-white relative overflow-hidden transition-colors duration-300">
      ...
      <div class="z-10 h-full flex flex-col justify-between">
        ...
        <div class="flex items-center gap-3 mb-8">
          <img :src="isDarkMode ? '/callbox-logo-white.svg' : '/callbox-logo.svg'" alt="Callbox" class="h-10" />
        </div>
        ...
        <div class="relative flex-1 flex flex-col justify-center">
          <transition enter-active-class="transition-all duration-700 ease-out"
            ...
            <div :key="currentSlide" class="flex flex-col justify-center">
              ...
              <div class="mt-12">
                <div
                  class="bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm p-6 rounded-xl border border-gray-200 dark:border-gray-700/50 shadow-xl inline-block max-w-sm transform hover:scale-105 transition-transform duration-300">
                  <div class="flex items-start gap-4">
                    ...
                    <div>
                      <h3 class="font-semibold text-gray-900 dark:text-white mb-1">{{ slides[currentSlide].featureTitle
                        }}</h3>
                      <p class="text-sm text-gray-500 dark:text-gray-400">{{ slides[currentSlide].featureDesc }}</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div class="mt-8 flex gap-3 z-10">
          <button v-for="(_, index) in slides" :key="index" @click="currentSlide = index"
            ...
            :class="currentSlide === index ? 'w-8 bg-amber-500' : 'w-2 bg-gray-300 dark:bg-gray-700 hover:bg-gray-400 dark:hover:bg-gray-600'"></button>
        </div>
      </div>
    </div>
    ...
    <div
      class="flex flex-col items-center justify-center p-8 bg-white dark:bg-gray-950 text-gray-900 dark:text-white relative transition-colors duration-300">
      <div class="w-full max-w-sm">
        <div class="mb-10 text-center md:text-left">
          <h2 class="text-3xl font-bold mb-2">Welcome back</h2>
          <p class="text-gray-500 dark:text-gray-400">Please sign in to access your sessions.</p>
        </div>
        ...
        <div class="flex flex-col gap-4">
          <div class="w-full">
            ...
            <button @click="handleGoogleLogin" :disabled="isVerifying || isSuccess"
              class="w-full flex items-center justify-center relative font-medium py-3 px-4 rounded-full transition-all focus:ring-4 focus:outline-none disabled:cursor-not-allowed overflow-hidden shadow-sm hover:shadow-md active:scale-95 duration-300 bg-white dark:bg-gray-800 text-gray-900 dark:text-white border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 focus:ring-gray-200 dark:focus:ring-gray-900">
              <!-- Success State -->
              <div v-if="isSuccess"
              ...
              <div v-if="isVerifying"
                class="absolute inset-0 bg-gray-100 dark:bg-gray-800 flex items-center justify-center z-10 font-medium text-gray-700 dark:text-gray-300 gap-2">
                <svg class="animate-spin h-5 w-5 text-gray-700 dark:text-gray-300" xmlns="http://www.w3.org/2000/svg"
          ...
          <div v-if="errorMsg"
            class="p-3 text-sm text-red-500 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/50 rounded-md">
            {{ errorMsg }}
          </div>
          ...
          <div class="relative my-6" v-else>
            ...
            <div class="relative flex justify-center text-sm">
              <span class="px-2 bg-white dark:bg-gray-950 text-gray-500">Secure Access</span>
            </div>
          </div>
        </div>
